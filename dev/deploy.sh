...
function get-export {
  jq --exit-status --raw-output --arg name "$STACK_PREFIX-$1" '.[] | select(.Name == $name) | .Value' <<< 
"$exports"
}
...
function build-frontend-image {
  ...
  repo=$(aws ecr describe-repositories --repository-names $ECR_REPO --output text --query 
