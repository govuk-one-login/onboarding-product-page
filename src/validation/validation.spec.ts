...
describe('Validation tests', function () {
    ...
    it('returns 12 errors if the form is empty', function () {
        assert.equal(new Validation({}, requiredFields, requiredConditionalFields).validate().size, 12, 'expected to see 12 errors');
    });

    it('does not accept foo@bar.com as a valid email address because it is not a government address', function () {
        let emailError = new Validation({email: "foo@bar.com"}, requiredFields, requiredConditionalFields).validate().get("email")
    ...
    it('does not accept foo@bar as a valid email address because the domain is incomplete', function () {
        let emailError = new Validation({email: "foo@bar"}, requiredFields, requiredConditionalFields).validate().get("email")
        assert.equal(emailError,
