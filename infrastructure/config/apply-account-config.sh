...
function control-tower {
  deploy-config-stack control-tower --tags 
sse:stack-role=account-management "$@"
}
...
function domain {
  ...
  if [[ $(../aws.sh get-current-account-name) == production ]]; then
    ...
    for account in development build staging integration; do
      servers=$(gds aws di-onboarding-$account -- ../aws.sh 
get-stack-outputs domain-config HostedZoneNameServers) || continue
      params+=("${account@u}NameServers=$(jq --raw-output ".value" <<< 
"$servers")")
    done
  else
  ...
  deploy-config-stack domain --tags sse:stack-role=dns 
