...
function get-account-group {
  jq --arg name "$1" 'with_entries(select(.value | contains([{name: 
$name}]))) | flatten' aws-accounts.json
}

function get-account-group-name {
  jq --raw-output --arg name "$1" 'to_entries | .[] | select(.value | 
contains([{name: $name}])) | .key' aws-accounts.json
}
...
function get-previous-account-name {
  ...
  jq --raw-output --exit-status ".[$((idx - 1)):$idx] | map(.name) | .[]" 
<<< "$accounts" || return 0
}
...
function get-caller-identity {
  aws sts get-caller-identity --profile gds-users 2> /dev/null && return
