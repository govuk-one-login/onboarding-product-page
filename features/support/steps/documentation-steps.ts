...
Then('the left-hand navigation menu is displayed', async function () {
    let technicalDocumentation = await this.page.$x(`//nav[@class="side-navigation"]/ul[contains(@class, "govuk-list")]//a[@href="/documentation" and contains(text(), "Technical documentation")]`);
    assertElementFound(technicalDocumentation, 'Technical documentation');
    let userJourneyMaps = await this.page.$x(`//nav[@class="side-navigation"]/ul[contains(@class, "govuk-list")]//a[@href="/documentation/user-journeys" and contains(text(), "User journey maps")]`);
    assertElementFound(userJourneyMaps, 'User journey maps');
    let designRecommendations = await this.page.$x(`//nav[@class="side-navigation"]/ul[contains(@class, "govuk-list")]//a[@href="/documentation/design-recommendations" and contains(text(), "Design recommendations")]`);
